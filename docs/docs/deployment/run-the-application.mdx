...
public interface Gateway {
    CompletableFuture<Any> execute(Any arguments, Map<String, String> headers);
}
```

Note the return type: `CompletableFuture<Any>`, unlike the plain `Any` used by regular components.
...
Since the proxy returns a plain `CompletableFuture<Any>`, you're not limited to returning the raw payload as-is. You can register callbacks on it, such as `thenApply`, `thenCompose`, `thenAccept`, and so on, to transform the flow's result into whatever shape your endpoint actually needs to return.
For example, unpacking the typed payload and wrapping it in a proper response DTO:
...
public class PublishResource {
    ...
    public CompletableFuture<PublishResponse> publish(Any arguments) {
        return gateway.execute(arguments)
            .thenApply(result -> {
                try {
                    StatusPayload status = result.unpack(StatusPayload.class);
                    return new PublishResponse(status.getContent());
                } catch (InvalidProtocolBufferException e) {
                    throw new RuntimeException("failed to unpack status payload", e);
                }
            });
    }
}
